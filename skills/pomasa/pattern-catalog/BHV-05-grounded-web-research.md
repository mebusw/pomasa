...
AI Agents have access to **search tools** and **web fetch tools**. A common mistake is treating search result summaries as reliable information. However, search summaries are generated by search engines or AI, often containing inaccuracies, hallucinations, or outdated information. Only content actually read from the original web page can be considered reliable data.

*Note: For which specific tools to use and in what priority order, see [BHV-06 Configurable Tool Binding](./BHV-06-configurable-tool-binding.md).*
...
|-----------|---------|-------------------|----------|
| **Search tool** | Find potentially relevant URLs | **Low** - summaries are unreliable | Discovering sources, getting leads |
| **Web fetch tool** | Retrieve actual page content | **High** - original content | Obtaining content to preserve |

*For specific tool names and priorities, see [BHV-06](./BHV-06-configurable-tool-binding.md).*
...
Step 3: Fetch Original Content
        │
        │  Use web fetch tool (url) for each promising URL
        │
        ▼
...
Step 1: Search("China electric vehicle policy 2024")
...
Step 3: Fetch("https://www.miit.gov.cn/...")
...
Search("China electric vehicle market size 2024")
